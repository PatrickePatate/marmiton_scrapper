...
let excludedUrls = await db('recipes').select('url').then((rows) => {
    return rows.map((row) => {
        return row.url;
    })}
);
...
const crawler = new PlaywrightCrawler({
    ...
    async requestHandler({ request, page, enqueueLinks, log }) {
        ...
        if(request.loadedUrl.match('https:\/\/www\.marmiton\.org\/recettes\/recette_([A-Za-z0-9\-\_]+).aspx')) {
            ...
            let season = (await page.locator('span.modal__tag.recipe-header-modal__seasonal').textContent()).trim();
            ...
            let author = (await page.locator('.recipe-author-note__author-name').textContent());
...
Promise.all([await crawler.run(['https://www.marmiton.org/recettes/index/ingredient']), await fileDownload.run()])